...
while True:
  if location == "start":
    ...
    time.sleep(1)
      ...
      time.sleep(1)
      answer = int(input(послать(1) или сказать что ты приехал на встречу(2)));
      if(answer == 1):
        print("они явно опешили от такого, но они набросились на меня и один сразу напоролся стилет , у второго отлетела голова, третий не успел ничего сделать как упал с рассечёным животом");
      if(answer == 2):
        print("они посмеялись и схватив выкинули в окно. Матерясь я вскочил на крыльцо и достав меч вбежал внутрь, из за ярости я с закрутился и за раз убил троих")
    location = "linneta";
